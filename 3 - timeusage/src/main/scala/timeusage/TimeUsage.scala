...
object TimeUsage {
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
	...
	val workStart         = List("t05", "t1805")
    val primaryNeedsCols = columnNames.filter(col => primaryNeedsStart.filter(name => col.startsWith(name)).length > 0)
	  .map(name => new Column(name))
	val workCols         = columnNames.filter(col => workStart.filter(name => col.startsWith(name)).length > 0)
	  .map(name => new Column(name))
	val otherCols        = columnNames.filter(col => (primaryNeedsStart ++ workStart).filter(name => col.startsWith(name)).length == 0)
	  .map(name => new Column(name))
	(primaryNeedsCols, workCols, otherCols)
  }
